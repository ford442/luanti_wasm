...
* VoxeLibre, Mineclonia, or `minetest_game` as the *default* web game.
* 128 px / HD texture packs.
* Large MP4s inside `--preload-file`. Theater clips are runtime fetches
  precisely so the download cost falls only on visitors who walk into the
  theater.
